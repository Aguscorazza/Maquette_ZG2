...
float ui=0,cde=0,up=0;// Variables du correcteur
...
void loop() {
  ...
  if(state_acq == 1){//Demande d'acquisition
    ...
    ui=0;
    erreur_prec=0;
    previous_time_duree_acq=millis();
    previous_time_corr=millis();
    previous_time_acq=millis();
    previous_time_enc=millis();
    }
}
...
void envoiDonnees(void){
    sprintf(buffer, "T%luR%dP%dC%d", tps_duree_acq,(int)nb_tour_min, pos, completSimu); // Formater les données dans la chaîne de caractères
